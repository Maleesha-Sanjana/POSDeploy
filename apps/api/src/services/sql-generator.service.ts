...
export function assertSafeIdentifier(name: string, label = 'name'): string {
...
function buildSqlType(col: ColumnDefinition): string {
...
function buildColumnClause(col: ColumnDefinition): string {
...
export function generateAddColumnSql(tableName: string, column: ColumnDefinition): string {
  const table = assertSafeIdentifier(tableName, 'table name');
...
export function generateAddColumnsSql(tableName: string, columns: ColumnDefinition[]): string {
  return columns.map((col) => generateAddColumnSql(tableName, col)).join('\n\n');
}

export function validateCreateTableInput(input: CreateTableInput): void {
...
export function validateAddColumnInput(input: AddColumnInput): void {
  assertSafeIdentifier(input.table_name, 'table name');
