...
export async function debugRoutes(app: FastifyInstance) {
  ...
  app.post<{ Body: { filename: string; pos_ids: number[] | 'all' } }>('/api/debug/deploy', async (req, reply) => {
    const { filename, pos_ids } = req.body;

    if (!filename) return reply.status(400).send({ error: 'Filename is required' });

    let posMachines: PosMachine[] = [];
    if (pos_ids === 'all') {
      posMachines = getDb().prepare('SELECT * FROM pos_machines WHERE is_active = 1').all() as PosMachine[];
    } else if (Array.isArray(pos_ids) && pos_ids.length > 0) {
      const placeholders = pos_ids.map(() => '?').join(',');
      posMachines = getDb().prepare(`SELECT * FROM pos_machines WHERE id IN (${placeholders})`).all(...pos_ids) as PosMachine[];
    } else {
