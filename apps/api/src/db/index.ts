...
function seedDefaultScript(db: Database.Database) {
...
END`;
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?)
  `).run(
    'gen_usergroup_levels',
    'Add Level 1/2/3 records to gen_usergroup table',
    sql,
    'data'
  );
}

export function toPublicPos(row: Record<string, unknown>) {
