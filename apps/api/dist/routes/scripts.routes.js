...
export async function scriptRoutes(app) {
    ...
    app.put('/api/scripts/:id', async (req, reply) => {
        ...
        getDb()
          ...
          WHERE id = ?
        `)
            .run(name?.trim() ?? row.name, description?.trim() ?? row.description, sql_text?.trim() ?? row.sql_text, script_type ?? row.script_type, id);
