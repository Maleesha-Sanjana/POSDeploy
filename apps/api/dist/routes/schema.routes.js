...
import { generateAddColumnSql, generateCreateTableSql, validateAddColumnInput, validateCreateTableInput, } from '../services/sql-generator.service.js';
...
function insertColumn(tableName, schemaTableId, col) {
    const result = getDb()
        .prepare(`
    ...
    `)
        .run(schemaTableId, tableName, col.name.trim(), col.data_type.toUpperCase(), col.length ?? null, col.precision ?? null, col.scale ?? null, col.nullable === false ? 0 : 1, col.default_value ?? null, col.is_primary_key ? 1 : 0, col.is_identity ? 1 : 0);
...
export async function schemaRoutes(app) {
    ...
    app.post('/api/schema/tables', async (req, reply) => {
        const body = req.body;
        ...
        try {
            ...
            const table = getDb().prepare('SELECT * FROM schema_tables WHERE id = ?').get(schemaTableId);
            const columns = getDb()
                .prepare('SELECT * FROM schema_columns WHERE schema_table_id = ? ORDER BY id')
                .all(schemaTableId);
            if (!deploy) {
    ...
    app.post('/api/schema/columns', async (req, reply) => {
        const body = req.body;
        const deploy = body.deploy !== false;
        try {
    ...
    app.post('/api/schema/preview', async (req, reply) => {
    ...
    app.delete('/api/schema/tables/:id', async (req, reply) => {
        const id = Number(req.params.id);
        const result = getDb().prepare('DELETE FROM schema_tables WHERE id = ?').run(id);
        if (result.changes === 0)
            return reply.status(404).send({ error: 'Schema table not found' });
        return { success: true };
    });
