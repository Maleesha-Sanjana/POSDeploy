import { getDashboardStats, getDeployJob, listDeployJobs, startDeployJob, waitForJob, } from '../services/deploy.service.js';
export async function deployRoutes(app) {
    ...
    app.get('/api/deploy/:jobId', async (req, reply) => {
    ...
    app.post('/api/deploy/:jobId/wait', async (req, reply) => {
        const job = getDeployJob(req.params.jobId);
        if (!job)
            return reply.status(404).send({ error: 'Deploy job not found' });
